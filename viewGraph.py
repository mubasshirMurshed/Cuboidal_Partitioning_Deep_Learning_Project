...
img = ds1[idx + i][0]
data = ds2.dataset[idx + i].x
edges_src = ds2.dataset[idx + i].edge_index[0]
edges_dst = ds2.dataset[idx + i].edge_index[1]
...
for i in range(len(data)):
    ax2.add_patch(plt.Circle((data[i][0]*28, data[i][1]*28), 0.35, facecolor=str(data[i][2].item()), edgecolor="white", linewidth=0.2))

# Add edges
for i in range(0, len(edges_src), 2):
    # Get source cuboid information
    src_cuboid = data[edges_src[i]]
    x_src = src_cuboid[0]*28
    y_src = src_cuboid[1]*28
    col_src = src_cuboid[2]

    # Get destination cuboid information
    dst_cuboid = data[edges_src[i+1]]
